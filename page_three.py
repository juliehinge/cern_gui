...
class PageThree(tk.Frame, Pages):
    ...
    def __init__(self, parent, controller):
        ...
        ttk.Label(self, text="Upload your csv/txt file please", font = ("bold", 20)).grid(row=1, column=0, padx = (10,30), pady = (10,30), columnspan=3)
        ...
        self.path = StringVar(self, value=' '); self.text = ttk.Label(self, textvariable = self.path, foreground="red").grid(row=3, column=0, columnspan=5, pady = 5)
    ...
    def import_csv_data(self):
        ...
        print(csv_file_path)
        if "csv" not in csv_file_path and "txt" not in csv_file_path: # Making sure the user only uploads a csv file
            ...
            if len(csv_file_path) > 40:
                self.path.set(csv_file_path[-40:])    # Writing the path of the file
            else:
